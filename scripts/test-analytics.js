...
for (const [label, documentLike, analyticsLocation, expectedPageType, expectedPath] of [
  ...
  if (JSON.stringify(params) !== JSON.stringify(expected)) {
    throw new Error(`Ford Bronco ${label} vehicle-card analytics payload is invalid.`);
  }
  ...
  if (!trackEvent("vehicle_guide_click", params, windowLike)) {
    throw new Error(`Ford Bronco ${label} vehicle-card event was not sent.`);
  }
  const matchingCalls = calls.filter(
    ([command, eventName]) =>
      command === "event" && eventName === "vehicle_guide_click"
  ...
  if (
    ...
    throw new Error(`Ford Bronco ${label} click must send exactly one event.`);
  }
}
...
for (const [label, documentLike, analyticsLocation, expectedPageType, expectedPath] of [
  ...
  if (JSON.stringify(params) !== JSON.stringify(expected)) {
    throw new Error(`Jeep Gladiator ${label} vehicle-card analytics payload is invalid.`);
  }
  ...
  if (!trackEvent("vehicle_guide_click", params, windowLike)) {
    throw new Error(`Jeep Gladiator ${label} vehicle-card event was not sent.`);
  }
  const matchingCalls = calls.filter(
    ([command, eventName]) =>
      command === "event" && eventName === "vehicle_guide_click"
  ...
  if (
    ...
    throw new Error(`Jeep Gladiator ${label} click must send exactly one event.`);
  }
}
...
for (const [label, documentLike, analyticsLocation, expectedPageType, expectedPath] of [
  ...
  const params = eventParameters(
    {
      dataset: {
        analyticsLocation,
        vehicleSlug: "chevrolet-colorado"
      }
    },
    "vehicle_guide_click",
    documentLike
  ...
  const expected = {
    vehicle_slug: "chevrolet-colorado",
    cta_location: analyticsLocation,
    page_type: expectedPageType,
    page_path: expectedPath
  };
  ...
  if (JSON.stringify(params) !== JSON.stringify(expected)) {
    throw new Error(`Chevrolet Colorado ${label} vehicle-card analytics payload is invalid.`);
  }
  ...
  if (!trackEvent("vehicle_guide_click", params, windowLike)) {
    throw new Error(`Chevrolet Colorado ${label} vehicle-card event was not sent.`);
  }
  const matchingCalls = calls.filter(
    ([command, eventName]) =>
      command === "event" && eventName === "vehicle_guide_click"
  ...
  if (
    ...
    throw new Error(`Chevrolet Colorado ${label} click must send exactly one event.`);
  }
}
...
for (const [label, documentLike, analyticsLocation, expectedPageType, expectedPath] of [
  ...
  if (JSON.stringify(params) !== JSON.stringify(expected)) {
    throw new Error(`Ford Ranger ${label} vehicle-card analytics payload is invalid.`);
  }
  ...
  if (!trackEvent("vehicle_guide_click", params, windowLike)) {
    throw new Error(`Ford Ranger ${label} vehicle-card event was not sent.`);
  }
  const matchingCalls = calls.filter(
    ([command, eventName]) =>
      command === "event" && eventName === "vehicle_guide_click"
  ...
  if (
    ...
    throw new Error(`Ford Ranger ${label} click must send exactly one event.`);
  }
}
...
for (const [label, documentLike, analyticsLocation, expectedPageType, expectedPath] of [
  ...
  if (JSON.stringify(params) !== JSON.stringify(expected)) {
    throw new Error(`Toyota Tundra ${label} vehicle-card analytics payload is invalid.`);
  }
  ...
  if (!trackEvent("vehicle_guide_click", params, windowLike)) {
    throw new Error(`Toyota Tundra ${label} vehicle-card event was not sent.`);
  }
  const matchingCalls = calls.filter(
    ([command, eventName]) =>
      command === "event" && eventName === "vehicle_guide_click"
  ...
  if (
    ...
    throw new Error(`Toyota Tundra ${label} click must send exactly one event.`);
  }
}
...
for (const [label, documentLike, analyticsLocation, expectedPageType, expectedPath] of [
  ...
  if (JSON.stringify(params) !== JSON.stringify(expected)) {
    throw new Error(`Nissan Frontier ${label} vehicle-card analytics payload is invalid.`);
  }
  ...
  if (!trackEvent("vehicle_guide_click", params, windowLike)) {
    throw new Error(`Nissan Frontier ${label} vehicle-card event was not sent.`);
  }
  const matchingCalls = calls.filter(
    ([command, eventName]) =>
      command === "event" && eventName === "vehicle_guide_click"
  ...
  if (
    ...
    throw new Error(`Nissan Frontier ${label} click must send exactly one event.`);
  }
}
